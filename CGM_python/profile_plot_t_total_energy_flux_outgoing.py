...
#  print ("pro1 y=",pro1['total_energy_flux_radial'])
  ...
  y = pro1['total_energy_flux_radial'] * (pro3['cell_volume']/pro2['cell_volume']) 
#  print ("after pro1 y=",pro1['total_energy_flux_radial'])
  print ("x=",x)
  print ("y=",y)
#  profiles.append(pro1)
  time = (ds.current_time).in_units('Gyr')
  plt.plot(x,y, label="t = %.1f Gyr" % time, c=color[count])
...
count=0
color=cm.plasma(np.linspace(0,0.9, len(num)))
for i in num:
   see(i)
   print (a0[0])
   count = count+1
...
plt.ylim(1e46, 1e49)
plt.xlabel('r [kpc]')
plt.ylabel('total energy flux [erg/yr]')
plt.legend(loc=1)
plt.savefig("total_energy_flux_outgoing_t_"+str(num)+"_min10kpc.png")
