...
### TEMPERATURE with Line integral convolution
    ...
    s.annotate_line_integral_convolution('velocity_y', 'velocity_z',kernellen=30, lim=(0.4,0.65), alpha=0.6)
...
#    s.annotate_streamlines('velocity_z', 'velocity_y', field_color='velocity_magnitude', display_threshold=0.1, factor=8, density=4)
    s.annotate_streamlines('velocity_y', 'velocity_z', field_color=None, display_threshold=0.1, factor=8, density=4)
    s.annotate_sphere([0, 0, 0], radius=(1.00*r200m.value, "kpc"),
...
### DENSITY with LIC 
    ...
    s.hide_axes(draw_frame=True)
#     s.hide_colorbar()
    s.save('den_stream'+str(i),mpl_kwargs={'bbox_inches':'tight', 'dpi':400})

num=[200,250,300,350,400]
for i in num:
    see(i)
